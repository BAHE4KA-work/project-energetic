...
async def do_plural(data: DataInputScheme, data_id: Optional[int] = None, session: Session = Depends(get_db)):
    if data_id:
        data = session.query(RawData).filter_by(id=data_id).first()
    else:
        data = RawData(**data.__dict__)
    card = await ds.create_card(data, session, do_check=False)
    our_cr: int = card.level
    try:
        res = await cs.do_plural_check(our_cr, data)
